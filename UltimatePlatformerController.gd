...
func _process(_delta):
	...
	if (
		...
		and latch
		and wallLatching
		and ((wallLatchingModifer and latchHold) or !wallLatchingModifer)
	):
		latched = true
	else:
		latched = false
...
func _physics_process(delta):
	...
	if cornerCutting:
		if (
			velocity.y < 0
			and leftRaycast.is_colliding()
			and !rightRaycast.is_colliding()
			and !middleRaycast.is_colliding()
		):
